...
game = {
  ...
  handleDragAndDrop: function () {
    ...
    const dropHandler = (e) => {
      ...
      const optionCircle = document.getElementById(optionId);
      e.target.append(optionCircle);

      const optionsCount = this.questions.options.length;
      const currentDroppedOptions =
        this.lighterBox.children.length + this.darkerBox.children.length;

      if (currentDroppedOptions === optionsCount)
        this.submitButton.disabled = false;
    };

    this.lighterBox.addEventListener("dragover", (e) => e.preventDefault());
    this.darkerBox.addEventListener("dragover", (e) => e.preventDefault());
    this.lighterBox.addEventListener("drop", dropHandler);
    this.darkerBox.addEventListener("drop", dropHandler);
  },
